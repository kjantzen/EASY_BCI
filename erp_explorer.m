    ...
    for jj = 1:n_conds %this will be for comparing conditions
      ...
      %  end
        ...
        elseif jj==n_conds && has_stat
            cb = colorbar(my_h(pcount));
            cb.Units = 'normalized';
            cb.Position(1) = 1-(cb.Position(3) * 3)- .005;%h.panel.topomy_h(pcount).Position(1) + my_h(pcount).Position(3) + cb.Position(3);
            %cb.Position(1) = my_h(pcount).Position(1) + my_h(pcount).Position(3) + cb.Position(3);
...
if stacked
    if ~userScale
        mnAmp = min(min(min(d))) * 1.1;
        mxAmp = max(max(max(d))) * 1.1;
        if plotFFT && mnAmp > 0
            mnAmp = 0;
        end
    end
    h.axis_erp.YLim = [mnAmp, mxAmp];
    h.spinner_minamp.Value = mnAmp;
    h.spinner_maxamp.Value = mxAmp;
...
% *************************************************************************
function callback_resetxaxis(~,~,h)

p = h.figure.UserData;
isfft = h.menu_fft.Checked;
if isfft
    h.spinner_mintime.Value = p.EEG.freqs(1);
    h.spinner_maxtime.Value = p.EEG.freqs(end);
else
    h.spinner_mintime.Value = p.EEG.times(1);
    h.spinner_maxtime.Value = p.EEG.times(end);
end
...
handles.menu_resetxaxis.MenuSelectedFcn = {@callback_resetxaxis, handles};
